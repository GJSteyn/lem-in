...
/*   Updated: 2018/07/27 14:59:16 by gsteyn           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list				*init_rooms(t_list *in)
{
	...
	while (in)
	{
		if (is_comment((char *)in->content))
			...
			add_room(&ret, (char*)(in = in->next)->content, idx++, start);
		else if (is_end((char*)in->content))
			add_room(&ret, (char*)(in = in->next)->content, idx++, end);
